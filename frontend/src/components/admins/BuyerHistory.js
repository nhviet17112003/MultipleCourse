...
export default function BuyerHistory() {
  ...
  useEffect(() => {
    const fetchBuyerHistory = async () => {
      try {
        const res = await axios.get("http://localhost:3000/api/orders/all-purchase-course", {
  ...
  return (
    <div className="container mx-auto px-4 py-6">
      <h1 className="text-2xl font-bold text-center text-gray-800 mb-6">Buyer History</h1>
              ...
              <tr key={order._id} className="border-b hover:bg-gray-100 transition">
                <td className="py-3 px-4">
                  {order.buyers.map((buyer, index) => (
                    <div key={index} className="text-gray-700">{buyer.email}</div>
                  ))}
                </td>
                <td className="py-3 px-4 text-gray-700">{order.course.title}</td>
                <td className="py-3 px-4 text-gray-700">${order.course.price}</td>
                <td className="py-3 px-4 text-gray-700">{new Date(order.course.date).toLocaleDateString()}</td>
              </tr>
            ))}
          </tbody>
        </table>
